...
class CCAvenue:
    ...
    def __init__(self, working_key: str = None, access_code: str = None, merchant_code: str = None) -> None:
        """
    ...
    def load_working_key(self, working_key: str = None) -> None:
        ...
        if self.__WORKING_KEY is None or isinstance(self.__WORKING_KEY, str) is False:
            raise ValueError('You must provide a working key for CCAvenue or set working key as an environment variable by setting it as CCAVENUE_WORKING_KEY.')
    ...
    def load_access_code(self, access_code: str = None) -> None:
        ...
        if self.__ACCESS_CODE is None or isinstance(self.__ACCESS_CODE, str) is False:
            raise ValueError('You must provide an access code for CCAvenue or set access code as an environment variable by setting it as CCAVENUE_ACCESS_CODE.')
    ...
    def load_merchant_code(self, merchant_code: str = None) -> None:
        ...
        if self.__MERCHANT_CODE is None or isinstance(self.__MERCHANT_CODE, str) is False:
            raise ValueError('You must provide a merchant code for CCAvenue or set merchant code as an environment variable by setting it as CCAVENUE_MERCHANT_CODE.')
    ...
    def validate_request_body(self) -> None:
        ...
        """
        if self.__form_data['order_id'] is None or isinstance(self.__form_data['order_id'], str) is False:
            raise ValueError('You must provide an order id for CCAvenue.')
        
        if self.__form_data['currency'] is None or isinstance(self.__form_data['currency'], str) is False:
        ...
        if self.__form_data['redirect_url'] is None or isinstance(self.__form_data['redirect_url'], str) is False:
            raise ValueError('You must provide a valid redirect url for CCAvenue.')
        
        if self.__form_data['cancel_url'] is None or isinstance(self.__form_data['cancel_url'], str) is False:
            raise ValueError('You must provide a valid cancel url for CCAvenue.')
        
        if self.__form_data['billing_name'] is None or isinstance(self.__form_data['billing_name'], str) is False:
            raise ValueError('You must provide a billing name for CCAvenue.')
        
        if self.__form_data['billing_tel'] is None or isinstance(self.__form_data['billing_tel'], str) is False:
            raise ValueError('You must provide a billing tel for CCAvenue.')
        
        if self.__form_data['billing_email'] is None or isinstance(self.__form_data['billing_email'], str) is False:
            raise ValueError('You must provide a billing tel for CCAvenue.')
    ...
    def __get_cipher(self) -> AES:
        ...
        return AES.new(hashlib.md5(bytearrayWorkingKey).digest(), AES.MODE_CBC, self.__iv.encode("utf-8"))
    ...
    def parse_response_body(self, response_body: str) -> str:
        ...
        """
        if response_body.get("encResp") is None or isinstance(response_body.get("encResp"), str) is False:
